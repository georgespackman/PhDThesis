...
void CTextileMaterials::OutputMaterials(ostream &Output, int iNumYarns, bool bMatrixOnly )
{
	...
	for (i = -1; i < iNumYarns; ++i)
	{
		...
		if ( i == -1 )
		{
			Output << "*Solid Section, ElSet=Matrix, Material=" << MatName << ", controls=HourglassEnhanced" << endl;
			Output << "1.0," << endl;
		}
		else if (!bMatrixOnly)
		{
		...
		Output << "*SECTION CONTROLS, NAME=HourglassEnhanced, HOURGLASS=ENHANCED" << endl;
	}	
}
