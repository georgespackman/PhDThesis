...
/*void COctreeVoxelMesh::OutputTJointBoundaries(ostream &Output, CTextile& Textile, int iBoundaryConditions, bool bMatrixOnly)
{
	vector<int> FlangeANodes, FlangeBNodes, WebNodes;
	CPrismDomain *Domain = Textile.GetPrismDomain();
...
void COctreeVoxelMesh::SaveVoxelMesh(CTextile &Textile, string OutputFilename, int XVoxNum, int YVoxNum, int ZVoxNum, int min_level, int refine_level, bool smoothing, int iter, double s1, double s2, bool surfaceOutput)
{
	...
	CVoxelMesh::SaveVoxelMesh(Textile, OutputFilename, 1, 1, 1, true, true, NO_BOUNDARY_CONDITIONS, TJOINT_CONDITIONS);

	int iTJointConditions = TJOINT_CONDITIONS;
	bool bMatrixOnly = false;
	ofstream Output(OutputFilename.c_str(), ios::out | ios::app);
...
void COctreeVoxelMesh::OutputTJointBoundaries(ostream & Output, CTextile & Textile, int iTJointConditions, bool bMatrixOnly)
{
	vector<int> boundaryPoints;
	boundaryPoints.reserve(boundaryPoints.size());
	for (int i=0 ; i != m_boundaryPoints.size(); i++)
	{
		boundaryPoints.push_back(m_boundaryPoints[i].nodeNum);
	}
	...
	{
		for (itNodes = m_AllElements[i].begin(); itNodes != m_AllElements[i].end(); itNodes++)
		{
			numberofNodes++;
			if (find(boundaryPoints.begin(), boundaryPoints.end(), *itNodes) != boundaryPoints.end())
			{
				m_TboundaryPoints.push_back(*itNodes);
			}
		}
	}
	sort(m_TboundaryPoints.begin(), m_TboundaryPoints.end());
	m_TboundaryPoints.erase(unique(m_TboundaryPoints.begin(), m_TboundaryPoints.end()), m_TboundaryPoints.end());

	m_numberofNodes = numberofNodes;
	double x, y, z;
	vector<int> WebNodes, Flange_A_Nodes, Flange_B_Nodes;
	...
	// The code checks if the point belongs to the top of the web or side of the flanges
	for (int i = 0; i < m_boundaryPoints.size(); i++) {
		x = (double)m_boundaryPoints[i].x;
		y = (double)m_boundaryPoints[i].y;
		z = (double)m_boundaryPoints[i].z;

		if (my_comparison(x, x_min) && find(m_TboundaryPoints.begin(), m_TboundaryPoints.end(), boundaryPoints[i]) != m_TboundaryPoints.end())
		{
			WebNodes.push_back(m_boundaryPoints[i].nodeNum);
		}
		if (my_comparison(z, z_max) && find(m_TboundaryPoints.begin(), m_TboundaryPoints.end(), boundaryPoints[i]) != m_TboundaryPoints.end())
		{
			Flange_A_Nodes.push_back(m_boundaryPoints[i].nodeNum);
		}
		if (my_comparison(z, z_min) && find(m_TboundaryPoints.begin(), m_TboundaryPoints.end(), boundaryPoints[i]) != m_TboundaryPoints.end())
		{
			Flange_B_Nodes.push_back(m_boundaryPoints[i].nodeNum);
		}

	}

	m_TJointBoundaries.SetWebNodes(WebNodes);
	m_TJointBoundaries.SetFlangeANodes(Flange_A_Nodes);
	m_TJointBoundaries.SetFlangeBNodes(Flange_B_Nodes);
